...
- **Running the app while testing**: kill old instances first (`pkill -f target/debug/gpui-issue-tracker-ui`). A stale instance keeps its window on top and will silently be the thing you screenshot, which looks exactly like a bug in the code you just changed. Also run `cargo build`, not just `cargo check`, before launching the binary.
